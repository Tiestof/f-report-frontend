...
import { crearReporte, actualizarReporte, ESTADO_ELIMINADO } from '../../services/reportesService';
...
const schemaEdit = schemaBase.extend({
  ...
  hora_fin: z.string().min(1, 'Hora de tÃ©rmino requerida en ediciÃ³n'),
...
const ReporteFormTecnico: React.FC<Props> = ({ mode, id_reporte, initialValues, onSaved }) => {
  ...
  const buildDefaultValues = useCallback((): FormValues => ({
    fecha_reporte: initialValues?.fecha_reporte
  ...
  const defaultValues = useMemo<FormValues>(() => buildDefaultValues(), [buildDefaultValues]);
  ...
  const estadosDisponibles = useMemo(() => (
    isEdit ? estados.filter((e) => e.id_estado_servicio !== ESTADO_ELIMINADO) : estados
  ), [estados, isEdit]);
  ...
  }, [mode, buildDefaultValues, form]);

  const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-3 flex items-center gap-2">{children}</h3>
  );
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values) => {
    try {
      ...
      if (mode === 'create') {
        ...
        throw new Error('Modo de formulario no soportado para guardar.');
      }

      onSaved?.();
    } catch (error) {
      console.error('Error guardando reporte t?cnico', error);
      toast.error('No fue posible guardar el reporte');
      throw error;
    }
  };
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div>
          <SectionTitle>👤 Datos cliente</SectionTitle>
      ...
      <ConfirmSaveModal
        ...
        onConfirm={async () => {
          try {
            ...
            // Se mantiene el modal abierto para que el usuario revise el error
          }
        }}
